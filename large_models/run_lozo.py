...
class OurArguments(TrainingArguments):
    ...
    lozo_random_device: str = (
        "cuda"  # cpu or cuda; controls where LOZO U/V/z random tensors are sampled
    )
    lozo_train_scope: str = "lora_normal"  # lora_normal skips embeddings/1D; full includes embeddings and 1D params
    lozo_perturbation_backend: str = (
        "dense"  # dense or lora; lora avoids dense probe writes
    )
    lozo_update_backend: str = (
        "dense"  # dense or lazy_lora; lazy_lora accumulates updates in LoRA form
    )
    ...
    eval_accuracy_during_training: bool = (
        False  # compute task accuracy at each HF eval step
    )
...
class EvalAccuracyCallback(TrainerCallback):
    ...
    def on_evaluate(self, args, state, control, metrics=None, **kwargs):
        ...
        if getattr(args, "eval_accuracy_during_training", False):
            acc_metrics = self.framework.evaluate(
                [], self.eval_samples, verbose_examples=0
            )
...
class Framework:
    ...
    def train(self, train_samples, eval_samples):
        ...
        if os.path.isdir(self.args.output_dir) and not getattr(
